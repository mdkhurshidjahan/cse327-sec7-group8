$(function(){
	$(document).on('click','.addTweetBtn',function(){
		...
		$.post('http://localhost/Connectify/core/ajax/tweetForm.php',function(data){
	...
	$(document).on('submit','#popupForm',function(e){
		...
		$.ajax({
			url: "http://localhost/Connectify/core/ajax/addTweet.php",
			type: "POST",
			data: formData,
			success: function(data){
				result = JSON.parse(data);
				if(result['error']){
					$('<div class="error-banner"><div class="error-banner-inner"><p id="errorMsg"><br>'+result.error+'<br></p></div></div>').insertBefore('.header-wrapper');
					$('.error-banner').hide().slideDown(300).delay(5000).slideUp(300);
					$('.popup-tweet-wrap').hide();
				} else if(result['success']){
					$('<div class="error-banner"><div class="error-banner-inner"><p id="errorMsg"><br>'+result.success+'<br></p></div></div>').insertBefore('.header-wrapper');
					$('.error-banner').hide().slideDown(300).delay(5000).slideUp(300);
					$('.popup-tweet-wrap').hide();
				}
			},
